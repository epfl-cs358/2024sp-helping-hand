...
Each box will be equipped with one ESP32 that will host an expose a \footlink{Web API}{https://en.wikipedia.org/wiki/Web_API} to interact with the servos from the smartphone application via the home Wi-Fi network.
...
We also have an additional goal of making the hardware of the project look and feel like a real product at the end of the course.
We do not only want a system that is  functional, but we are really looking towards having it look slick and be as small as possible.
Of course, we will start by implementing a working version first and then work our way up to reducing its size and improving the end user experience.

% TODO add and comment sketches

\textbf{Software}

The software side of the project will be implemented using \footlink{Arduino}{https://www.arduino.cc/} programming to control the ESP32 microcontroller and servomotors.
...
As we want to support many remote encasings per network, the protocol has to assure correct communication between all the ESP32 and the smartphone application.
